		...
		<div class="session-right text-center">
			<div class="session-table-cell">
				<div class="session-content">
					...
					<template v-else>
						<p class="fs-14">Ha sido enviado un correo al siguiente correo electrónico</p>
	...
	methods: {
		...
		sendEmail(){
			this.$refs.forgotPasswordForm.validate().then(success => {
                if (success == true) {
                    ...
                    this.$store.dispatch('users/sendForgottenPasswordEmail', formData).then(response => {
						...
						if (response.data.status == 'OK') {
							...
							setTimeout(() => (this.$router.push('/session/reset-password')), 4000);
						}
						else
				...
				{  this.$swal({icon: 'info', title: 'Oops...', text: 'Debe ingresar un email válido.',});  }
