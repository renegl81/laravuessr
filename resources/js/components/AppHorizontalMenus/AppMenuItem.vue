...
<template>
   <li class="nav-item">
            ...
            <ul class="list-unstyled sub-menu-child" :class="{'deep-level': menu.items.length > 10 }">
               <template v-for="subMenu in menu.items">
                  <li class="nav-item" :key="subMenu.title">
                     <router-link :to="!subMenu.exact ? `/${getCurrentAppLayoutHandler() + subMenu.path}` : subMenu.path" class="nav-link" activeClass="active">
                        {{$t(subMenu.title)}}
         ...
         <template v-else>           
            <template>
               <router-link :to="!menu.exact ? `/${getCurrentAppLayoutHandler() + menu.path}` : menu.path" class="nav-link no-arrow" activeClass="active">
                  <i class="zmdi" :class="menu.action"></i>
                  {{$t(menu.title)}}
                  <template  v-if = "menu.label == 'New'">
                     <span class="sidebar-label">{{menu.label}}</span>
