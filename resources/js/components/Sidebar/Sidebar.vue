	...
	<div class="sidebar" :class="sidebarSelectedFilter.class">
        ...
        <div class="transparent navigation">
          <v-list>
			...
			<template v-for="(category, key) in menu">
				<div :key="key">
					...
					<template v-for="item in category">
						<template v-if="item.items != null">
							<v-list-group :ripple="false" :key="item.title"	prepend-icon="arrow_right" append-icon="" no-action v-model="item.active">
								...
								<v-list-item :ripple="false" v-for="subItem in item.items" v-bind:key="subItem.title" :to="!subItem.exact ? `/${getCurrentAppLayoutHandler() + subItem.path}` : subItem.path">
						...
						<template v-else>
							<v-list-group :ripple="false" class="not-submenu" :key="item.title"	prepend-icon="arrow_right" append-icon="" no-action	v-model="item.active">
								<v-list-item slot="activator" :ripple="false">
									<v-list-item-content >
										<v-list-item-title>
...
export default {
  ...
  created() {
    this.$store.dispatch('sidebar/getSidebarMenu');
  },
  computed: {
	...mapGetters({
		menu: 'sidebar/menu',
